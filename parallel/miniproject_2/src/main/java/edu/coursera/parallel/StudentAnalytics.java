...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        return Arrays.stream(studentArray).filter(s -> s.checkIsCurrent()).mapToDouble(s -> (s.getAge())).average().getAsDouble();


//        double sum = Arrays.stream(studentArray).filter(s -> s.checkIsCurrent()).mapToDouble(s -> (s.getAge())).sum();
//        System.out.println(sum + "sum");
//        double cnt =Arrays.stream(studentArray).count();
